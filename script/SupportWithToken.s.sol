...
contract SupportWithToken is Script {
    function run() public {
        ...
        uint256 amount = vm.envUint("SUPPORT_AMOUNT");
        bytes memory data = abi.encode(vm.envString("SUPPORT_PARAMS"));

        vm.startBroadcast(viewerPK);
        token.approve(address(sf), amount);
        sf.supportWithToken(streamer, address(token), amount, data);
        vm.stopBroadcast();

        console2.log("Successfully support to ", streamer, " with ", amount);
    }
}
